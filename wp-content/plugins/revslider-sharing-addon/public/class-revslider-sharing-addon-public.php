		...
		if($ouput_obj !== '') $tabs .= $ouput_obj->ld();
		 ...
		 switch ($actionType) {
			case 'share_twitter':
				...
				switch ($a_sharelink) {
					case '%site_url%':
						...
						break;
					case '%post_url%':
						//$a_sharetext = $a_sharetext."tp_revslider_sharing_get_the_title"; //get_the_title( $slide->get_id() );
				...
				$sharetext = empty($a_sharetext) ? $sharelink : $a_sharetext.' '.$sharelink;
				$html_simple_link = 'target="_blank" ' . "\n" . $tabs . 'href="'.$caller_url.'?tpurl='.$url.'&share=https://twitter.com/home?status='.urlencode($sharetext).'&slider='.$slide->getSliderID().'&source='.$source.'"';
				break;
			case 'share_facebook':
			...
			case 'share_pinterest':
				...
				$html_simple_link = 'target="_blank" ' . "\n" . $tabs . 'href="'.$caller_url.'?tpurl='.$url.'&share=https://pinterest.com/pin/create/button/?url='.urlencode($sharelink).',media='.urlencode($shareimage).',description='.urlencode($sharedesc).'&slider='.$slide->getSliderID().'&source='.$source.'"';
				break;
			case 'share_linkedin':
				...
				$html_simple_link = 'target="_blank" ' . "\n" . $tabs . 'href="'.$caller_url.'?tpurl='.$url.'&share=https://www.linkedin.com/shareArticle?mini=true,url='.urlencode($sharelink).',title='.urlencode($sharetitle).',summary='.urlencode($sharesummary).'&slider='.$slide->getSliderID().'&source='.$source.'"';
				break;
			default:
				break;
		}
